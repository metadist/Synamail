...
| HTTP               | `fetch` + Zod schemas — _re-use generated schemas_ from `synaplan/frontend/src/generated/api-schemas.ts` so the contract stays in sync with the backend OpenAPI spec                        | Same pattern documented in `docs/API_PATTERNS.md`                                                                    |
...
├── README.md
├── AGENTS.md                     workspace rules for agents and humans
├── LICENSE
├── Makefile                      make help, ci-local, sideload, …
├── manifest.xml                  Outlook XML manifest (primary)
├── manifest.unified.json         Unified Microsoft 365 manifest (Sprint 4; generated)
├── package.json
├── tsconfig.json                 + tsconfig.app.json / node.json / vitest.json
├── vite.config.ts                + vitest.config.ts + playwright.config.ts
├── eslint.config.js              + commitlint.config.cjs + .prettierrc.json
├── docs/                         architecture, features, glossary, commit process, integration map
│   ├── ARCHITECTURE.md           (this file)
│   ├── COMMIT_PROCESS.md
│   ├── CONTRIBUTING.md
│   ├── FEATURES.md
│   ├── GLOSSARY.md
│   └── SYNAPLAN_INTEGRATION.md
├── planning/                     sprints, steps, status, GUI spec, AppSource gate
│   ├── PLAN.md
│   ├── STATUS.md
│   ├── STEPS.md
│   ├── GUI_DEFINITIONS.md
│   └── APPSOURCE_CHECKLIST.md
├── assets/                       icons (16/32/64/80/128), screenshots, store copy
│   ├── icon-*.png
│   └── store/                    AppSource hero / video / privacy URLs
├── src/
│   ├── i18n.ts                   vue-i18n bootstrap
│   ├── locales/                  en.json / de.json (Sprint 4 adds fr/es/it)
│   ├── shared/
│   │   ├── synaplan-client.ts    fetch wrapper, X-API-Key, Real + Mock + factory
│   │   ├── prompts.ts            reply / summarise / classify prompt templates
│   │   └── types.ts              narrow types (Sprint 3 adds Zod schemas)
│   ├── taskpane/
│   │   ├── taskpane.html
...
The user wrote: _"can NAA be done with a simple login to Synaplan (maybe via Google login) and the accounts are all connected?"_ — that conflates two different things, so to be precise:
...
- In phase 2 (NAA): if the M365 mailbox email _does_ match a Synaplan account (any provider), NAA links them silently. If the user wants a different mapping, they sign out and use the dialog flow.
